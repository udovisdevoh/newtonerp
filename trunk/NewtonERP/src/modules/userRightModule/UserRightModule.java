...
public class UserRightModule extends Module {
    ...
    public void initDB() {
        ...
        groupsID = (Integer) ((Groups) Orm.getInstance().select(groups, search)
                .get(0)).getData(groups.getPrimaryKeyName());
        ...
        for (AbstractOrmEntity currentRight : Orm.getInstance().select(
                new Right(), search)) {
    ...
    public void addGroupsRight(String groupName, String actionName,
            String entityName) {
        Groups groups = (Groups) Orm.getInstance().getOrCreateEntity(
                new Groups(), "groupName", groupName);
        Right right = (Right) Orm.getInstance().getOrCreateEntity(new Right(),
                "actionName", actionName, "entityName", entityName);

        Orm.getInstance().getOrCreateEntity(new GroupsRight(),
                groups.getForeignKeyName(),
                groups.getPrimaryKeyValue().toString(),
                right.getForeignKeyName(),
...

    {
        ...
        Orm.getInstance().delete(new GroupsRight(), groups.getForeignKeyName(),
                groups.getPrimaryKeyValue().toString(),
                right.getForeignKeyName(),
                right.getPrimaryKeyValue().toString());
    }
