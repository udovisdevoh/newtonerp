...
public class TaskModule extends Module {
	...
	public TaskModule() {
		...
		addGlobalActionMenuItem("Modules", new BaseAction("GetList", new ModuleEntity()));
		addGlobalActionMenuItem("Tâches", new BaseAction("GetList", new TaskEntity()));
		addGlobalActionMenuItem("Spécification", new BaseAction("GetList", new Specification()));
		addGlobalActionMenuItem("Effet", new BaseAction("GetList", new EffectEntity()));
		addGlobalActionMenuItem("Entités de recherches", new BaseAction("GetList", new SearchEntity()));
		addGlobalActionMenuItem("Critères de recherches", new BaseAction("GetList", new SearchCriteria()));
		addGlobalActionMenuItem("Paramètres", new BaseAction("GetList", new Parameter()));
	...
	private static void intiDynamicFieldTask() {
		...
		fieldEntity = (EntityEntity) Orm.getInstance().selectUnique(fieldEntity);

		SearchCriteriaOperator equals = new SearchCriteriaOperator();
		equals.setData("name", "=");
		equals = (SearchCriteriaOperator) Orm.getInstance().selectUnique(equals);
		...
		action = (ActionEntity) Orm.getInstance().selectUnique(action);
	...
	private static void initInvoiceLineTask() {
		...
		invoiceLineEntity = (EntityEntity) Orm.getInstance().selectUnique(invoiceLineEntity);
	...
	private static void initShippingTask() {
		...
		shippingEntity = (EntityEntity) Orm.getInstance().selectUnique(shippingEntity);
	...
	private static void initNewSupplierTransactionTask() {
		...
		finances = (ModuleEntity) Orm.getInstance().selectUnique(finances);

		EntityEntity invoice = new EntityEntity();
		invoice.setData("systemName", "Invoice");
		invoice = (EntityEntity) Orm.getInstance().selectUnique(invoice);
	...
	private static void initWorkOrderTask() {
		...
		mrmngmt = (ModuleEntity) Orm.getInstance().selectUnique(mrmngmt);

		EntityEntity product = new EntityEntity();
		product.setData("systemName", "Product");
		product = (EntityEntity) Orm.getInstance().selectUnique(product);
	...
	private static void initWorkOrderClosingTask() {
		...
		wo = (EntityEntity) Orm.getInstance().selectUnique(wo);
	...
	private static void initInvoiceTaxLineTask() {
		...
		invoiceTaxLineEntity = (EntityEntity) Orm.getInstance().selectUnique(invoiceTaxLineEntity);
	...
	private static void initEmployeeNewUserTask() {
		...
		parameter.setData("value", "Voici votre nouveau compte utilisateur :Employee.firstName:Employee.lastName");
		...
		entity = (EntityEntity) Orm.getInstance().selectUnique(entity);

		SearchEntity searchEntity = new SearchEntity();
		searchEntity.setData("name", "Employe sans login");
		searchEntity.setData(entity.getForeignKeyName(), entity.getPrimaryKeyValue());
		searchEntity.newE();

		User user = new User();
		user.setData("name", "unLogedUser");
		user = (User) Orm.getInstance().selectUnique(user);

		SearchCriteria searchCriteria = new SearchCriteria();
		searchCriteria.setData("key", new User().getForeignKeyName());
		searchCriteria.setData(searchEntity.getForeignKeyName(), searchEntity.getPrimaryKeyValue());
		searchCriteria.setData(new SearchCriteriaOperator().getForeignKeyName(), equals.getPrimaryKeyValue());
		...
		effect.setData(searchEntity.getForeignKeyName(), searchEntity.getPrimaryKeyValue());
		...
		specification.setData(searchEntity.getForeignKeyName(), searchEntity.getPrimaryKeyValue());
		specification.newE();

		TaskEntity task = new TaskEntity();
		task.setData("isActive", false);
		task.setData("straightSearch", false);
		task.setData(specification.getForeignKeyName(), specification.getPrimaryKeyValue());
	...
	private static void initActionsAndEntities() {
		...
		actionEntity.setData(moduleForeignKey, moduleEntity.getPrimaryKeyValue());
		...
		for(String moduleName : allModule){
			...
			for(String entityName : module.getEntityDefinitionList().keySet()){
				AbstractOrmEntity realEntity = module.getEntityDefinitionList().get(entityName);

				entityEntity = new EntityEntity();
				entityEntity.setData(moduleForeignKey, moduleEntity.getPrimaryKeyValue());
				entityEntity.setData("systemName", entityName);
				entityEntity.setData("visibleName", realEntity.getVisibleName());
				entityEntity.newE();

				initFieldEntitiesForEntityEntity(entityEntity, realEntity);
				initAccessorEntityListForEntityEntity(entityEntity, realEntity.getAccessorNameList());
			}
		}
	}

	private static void initAccessorEntityListForEntityEntity(EntityEntity entityEntity, Vector<String> accessorNameList)
	...
	private static void initFieldEntitiesForEntityEntity(EntityEntity entityEntity, AbstractOrmEntity realEntity)
...

	{
		...
		Vector<AbstractOrmEntity> result = Orm.getInstance().select(fieldType);
