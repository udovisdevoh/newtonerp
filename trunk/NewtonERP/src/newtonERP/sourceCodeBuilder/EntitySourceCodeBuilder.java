...
public class EntitySourceCodeBuilder {
	...
	public static String build(EntityEntity entityEntity) {
		...
		sourceCode += "package modules." + getPackageName(entityEntity) + ".entityDefinitions;\n";
		sourceCode += "\n";
		...
		sourceCode += " */\n";
		sourceCode += "public class " + getSystemName(entityEntity) + " extends AbstractOrmEntity\n";
		sourceCode += "{\n";
		sourceCode += "    /**\n";
		...
		sourceCode += "    {\n";
		sourceCode += "        super();\n";
		sourceCode += "        setVisibleName(\"" + getVisibleName(entityEntity) + "\");\n";
...

	{
		...
		PluralAccessor accessorList = entityEntity.getPluralAccessor("AccessorEntity");

		for(AbstractOrmEntity entity : accessorList){
			AccessorEntity accessorEntity = (AccessorEntity) entity;
			sourceCode += "        AccessorManager.addAccessor(this, new " + getForeignEntityName(accessorEntity)
			        + "());\n";
		}
	...
	private static String getFieldCode(FieldEntity fieldEntity, EntityEntity entityEntity) {
		String sourceCode = "";
		...
		sourceCode += "\n";
		sourceCode += "        " + getFieldType(fieldEntity) + " " + camelCaseName + " = new "
		        + getFieldType(fieldEntity) + "(\"" + getVisibleName(fieldEntity) + "\", " + quotedShortName + ");\n";

		if(isNaturalKey(fieldEntity)){
			sourceCode += "        " + camelCaseName + ".setNaturalKey(true);\n";
		}
		...
		if(isDynamicField(fieldEntity)){
			sourceCode += "        " + camelCaseName + ".setDynamicField(true);\n";
		}
	...
	{
		FieldTypeEntity fieldType = (FieldTypeEntity) fieldEntity.getSingleAccessor(new FieldTypeEntity()
		        .getForeignKeyName());
	...
	{
		return ModuleSourceCodeBuilder.getPackageName(getModuleEntity(entityEntity));
	}

	private static ModuleEntity getModuleEntity(EntityEntity entityEntity)

	{
		return (ModuleEntity) entityEntity.getSingleAccessor(new ModuleEntity().getForeignKeyName());
	}
