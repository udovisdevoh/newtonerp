...
public class CommandRouteur {
	 ...
	 */
	public AbstractEntity routeCommand(String moduleName, String actionName, String entityName,
...

	{
		...
		if(entityName != null){
			...
			if((new RightCheck()).perform(rightParam) != null){
				...
				return new AlertEntity("Vous n'avez pas les droits pour faire cette action.");
			}
		}else{
			if((new RightCheck()).perform(rightParam) != null){
				// on fais l'Action demander par le user si les droit son
		...
		AbstractEntity taskManagerRetView = TaskManager.executeTasks(entityName, parameter);
		if(taskManagerRetView != null){
			retView = taskManagerRetView;
		}

		// On averti l'Orm de faire des backup si c'est le temps
		// TODO: placé sa ailleur
		Orm.getInstance().doBackupIfTimeIntervalAllows();
