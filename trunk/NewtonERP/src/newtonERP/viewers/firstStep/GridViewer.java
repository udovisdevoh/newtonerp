...
public class GridViewer {
	...
	public static String getHtmlCode(GridViewerData gridEntity) {
		...
		if(gridEntity instanceof ListViewerData){
			if(((ListViewerData) (gridEntity)).getSearchBar() != null){
				searchBarHtml = SearchBarViewer.getHtmlCode(((ListViewerData) (gridEntity)).getSearchBar());
				html += searchBarHtml;
			}

			if(((ListViewerData) (gridEntity)).getPageSelector() != null){
				pageSelectorHtml = PageSelectorViewer.getHtmlCode(((ListViewerData) (gridEntity)).getPageSelector());
				html += pageSelectorHtml;
			}
		}

		html += "<table class='ListViewerTable' border='0' cellpadding='3' cellspacing='0'>";
		html += getTableHeader(gridEntity.getHeader(), gridEntity.getLeftHeader().length > 0);
	...
	private static String getTableHeader(GridCaseData[] headerCase, boolean hasLeftHeader) {
		...
		for(int i = 0; i < headerCase.length; i++){
			...
			if(headerCase[i] != null){
				for(k = i; k < headerCase.length && headerCase[i].equals(headerCase[k]); k++){
					if(k != i){
						headerCase[k] = null;
					}
				}
				...
				html += "<th" + isHeader + colspan + ">" + getCase(headerCase[i]) + "</th>";
			}
		}
		html += "</tr>";
		return html;
...

	{
		...
		for(int i = 0; i < data.length; i++){
			...
			for(int j = 0; j < data[i].length; j++){
				...
				if(data[i][j] != null){
					if(i == 0 || (i > 0 && !data[i][j].equals(data[i - 1][j])) || !gridData.isSpanSimilar()){
						for(k = i; k < data.length && data[i][j].equals(data[k][j]) && gridData.isSpanSimilar(); k++){
							// nothing to do...
						}
						if(k - i > 1){
							rowspan = " rowspan='" + (k - i) + "'";
						...
						if(gridData.isColored() || data[i][j].isColored()){
							color = " background-color:" + ColorViewer.getColor(caseContent) + ";";
						}

						html.append("<td class='gridCell" + moneyStyleModifier + "' " + rowspan + "style='" + color
			...
			if(gridData instanceof ListViewerData){
				html.append(getSpecificButton(gridData.getSpecificActionButtonList(), ((ListViewerData) gridData)
				        .getEntity().get(i)));
			}else{
				html.append(getSpecificButton(gridData.getSpecificActionButtonList(), null));
			}
	...
	private static String getSpecificButton(Vector<ActionLink> actionLinks, AbstractEntity entity) {
		String html = "";
