...
public class FlagPool implements CheckListViewable {
	 ...
	 */
	public FlagPool(AbstractOrmEntity sourceEntityDefinition, String visibleDescription,
		...
		if(sourceEntityDefinition == null || foreignEntityDefinition == null){
			throw new RuntimeException("Source entity and target entity can't be null");
		}
	...
	public TreeMap<String, String> getAvailableElementList() {
		Vector<AbstractOrmEntity> entityList = Orm.getInstance().select(foreignEntityDefinition, null);
		...
		for(AbstractOrmEntity entity : entityList){
			...
			availableElementList.put(description, intermediateEntityDefinition.getSystemName() + "."
			        + intermediateKeyOut + "." + entity.getDataString(foreignKey));
		}
...

	{
		...
		for(AbstractOrmEntity entity : getPositivePluralIntermediateAccessor()){
			checkedElementList.add(intermediateEntityDefinition.getSystemName() + "." + intermediateKeyOut + "."
	...
	/**
	 * @return définition d'entité source (entité de départ dans laquelle on met un flagPool)
	 */
...

	{
		...
		try{
			intermediateEntityDefinition.setData(intermediateKeyIn, sourceKeyValue);
		}catch(FieldNotCompatibleException e){
			int sourceKeyValueNumber = Integer.parseInt(sourceKeyValue);
			intermediateEntityDefinition.setData(intermediateKeyIn, sourceKeyValueNumber);
		}

		Vector<AbstractOrmEntity> pluralAccessor = Orm.getInstance().select(intermediateEntityDefinition);
	 ...
	 */
	public PluralAccessor getPositivePluralForeignAccessor(AbstractOrmEntity searchCriteriaEntity) {
		// searchCriteriaEntity can be null

		PluralAccessor pluralForeignAccessor = new PluralAccessor(foreignEntityDefinition);
		...
		if(searchCriteriaEntity == null){
			...
			for(AbstractOrmEntity intermediateEntity : pluralIntermediateAccessor){
				foreignEntityDefinition.setData(foreignKey, intermediateEntity.getData(intermediateKeyOut));

				resultSet = Orm.getInstance().select(foreignEntityDefinition);
			...
			resultSet = Orm.getInstance().select(searchCriteriaEntity);

			AbstractOrmEntity criteredIntermediateEntityDefinition;
			try{
				criteredIntermediateEntityDefinition = intermediateEntityDefinition.getClass().newInstance();
			...
			for(AbstractOrmEntity result : resultSet){
				...
				criteredIntermediateEntityDefinition.setData(intermediateKeyOut, result.getPrimaryKeyValue());
				criteredIntermediateEntityDefinition.setData(intermediateKeyIn,
				        sourceEntityDefinition.getPrimaryKeyValue());

				intermediateResultSet = Orm.getInstance().select(criteredIntermediateEntityDefinition);
