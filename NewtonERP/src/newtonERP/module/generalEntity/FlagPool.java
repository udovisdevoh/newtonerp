...
public class FlagPool implements CheckListViewable {
    ...
    public TreeMap<String, String> getAvailableElementList() {
        Vector<AbstractOrmEntity> entityList = Orm.getInstance().select(
...

    {
        ...
        Vector<AbstractOrmEntity> pluralAccessor = Orm.getInstance().select(
    ...
    public PluralAccessor getPositivePluralForeignAccessor(
        ...
        if (searchCriteriaEntity == null) {
            ...
            for (AbstractOrmEntity result : resultSet) {
                ...
                intermediateResultSet = Orm.getInstance().select(
                        criteredIntermediateEntityDefinition);
