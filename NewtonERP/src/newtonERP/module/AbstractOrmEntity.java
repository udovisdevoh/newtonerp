...
public abstract class AbstractOrmEntity extends AbstractEntity {
...

    {
        ...
        resultSet = Orm.getInstance().select(searchEntity, searchParameters,
                limit, offset, orderBy);

        int totalRowCount = Orm.getInstance().count(searchEntity,
                searchParameters);
...

    {
        ...
        whereParameter.add(whereClause);
        try {
            retEntityList = Orm.getInstance().select(
                    this.getClass().newInstance(), whereParameter);
        } catch (InstantiationException e) {
            throw new RuntimeException(e);
        } catch (IllegalAccessException e) {
            throw new RuntimeException(e);
