...
public class TaskManager {
	 ...
	 */
	public static AbstractEntity executeTasks(AbstractOrmEntity entity, int primaryKeyValue) {
		...
		if(entity.getFields().containsFieldName(entity.getPrimaryKeyName())){
			if(primaryKeyValue != 0){
				entityParameters.put(entity.getPrimaryKeyName(), Integer.toString(primaryKeyValue));
			}
		}
		...
		if(entity.getFields().containsFieldName(entity.getPrimaryKeyName())){
			if(entity.getPrimaryKeyValue() != null){
				if(entity.getPrimaryKeyValue() != 0){
					entityParameters.put(entity.getPrimaryKeyName(), entity.getPrimaryKeyValue().toString());
				}
			}
		}
	...
	/**
	 * @param entity Entité pour laquelle on doit effecter des tâches s'il y a lieu
	 * @param entityParameters paramètres de l'entité
	 * @return entité viewable de retour
	 */
	public static AbstractEntity executeTasks(AbstractOrmEntity entity, Hashtable<String, String> entityParameters) {
	 ...
	 */
	public static AbstractEntity executeTasks(String entityName, Hashtable<String, String> entityParameters) {
		if(entityName == null){
			return null;
		}
		...
		for(TaskEntity task : concernedTaskList){
			if(task.isActive()){
				if(task.isSatisfied(entityParameters, task.isStraightSearch())){
					retEntity = task.execute(entityParameters, task.isStraightSearch());
				}
			}
		}
...

	{
		if(taskModuleEntityDefinitionNameList == null){
			...
			for(String entityDefinitionName : taskModule.getEntityDefinitionList().keySet()){
	...
	{
		Vector<TaskEntity> concernedTaskList = TaskCache.getConcernedTaskList(entityName);
...

	{
		...
		for(AbstractOrmEntity specificationEntity : specificationList){
			PluralAccessor currentAccessor = specificationEntity.getPluralAccessor("TaskEntity");
	...
	private static PluralAccessor getSpecificationEntityList(PluralAccessor searchEntityList) {
		PluralAccessor specificationEntityList = new PluralAccessor(new Specification());

		for(AbstractOrmEntity searchEntity : searchEntityList){
			PluralAccessor currentAccessor = searchEntity.getPluralAccessor("Specification");
	...
	/**
	 * @param searchEntities entités pour lesquelles on doit possiblement effectuer des tâches
	 * @param entityParameters paramètres de l'entité
	 */
	public static void executeTasks(Vector<AbstractOrmEntity> searchEntities, Hashtable<String, String> entityParameters) {
