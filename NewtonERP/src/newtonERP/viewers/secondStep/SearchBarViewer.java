...
public class SearchBarViewer {
	...
	public static String getHtmlCode(SearchBar searchBar) {
		...
		if(searchBar.getCurrentOrder() != null && searchBar.getCurrentOrder().length() > 0){
			html += "<option value='" + searchBar.getCurrentOrder() + "'>" + searchBar.getCurrentOrder() + "</option>";
		}

		for(String possibleOrderName : searchBar.getPossibleOrderNameList()){
			if(!possibleOrderName.equals(searchBar.getCurrentOrder())){
				html += "<option value='" + possibleOrderName + "'>" + possibleOrderName + "</option>";
			}
		}
		html += "</select> ";

		if(searchBar.getOffset() != 0){
			html += "<input type='hidden' name='offset' value='" + searchBar.getOffset() + "' />";
		}

		if(searchBar.getLimit() != 0){
			html += "<input type='hidden' name='limit' value='" + searchBar.getLimit() + "' />";
		}
