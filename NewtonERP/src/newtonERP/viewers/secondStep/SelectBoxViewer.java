...
public class SelectBoxViewer {
	 ...
	 */
	public static String getHtmlCode(SelectBoxViewable entity, String inputName, String inputValue, boolean isReadOnly)

	{
		String html = "";

		String getListLinkHtml = LinkViewer.getHtmlCode(new ActionLink(entity.getLabelName(), new BaseAction("GetList",
		...
		if(isReadOnly){
			if(inputValue == null || inputValue.equals("null") || inputValue.equals("0")){
				inputValue = "1";
			}
			...
			if(elements.get(inputValue) != null){
				html += "<option value=\"" + inputValue + "\">" + elements.get(inputValue) + "</option>";
			}

			for(String elementKey : elements.getKeyList()){
				if(!elementKey.equals(inputValue)){
					html += "<option value=\"" + elementKey + "\">" + elements.get(elementKey) + "</option>";
				}
			}
			html += "</select>";
		}
