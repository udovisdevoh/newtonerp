...
public class PromptViewer {
...

	{
		...
		html += "<form method=\"post\" action=\"" + promptData.getButtonAction().getUrlParam() + "\">";
		...
		if(data instanceof AbstractOrmEntity){
			if(ormEntity != null){
				for(GateWay gateWay : ormEntity.getGateWayList()){
					try{
						AbstractOrmEntity externalEntity = GateWayManager.getExternalEntity(gateWay, ormEntity);

						html += "\n<tr><td>" + externalEntity.getNaturalKeyName() + ": </td><td>"
						        + externalEntity.getNaturalKeyDescription() + "</td></tr>";
					}catch(Exception e){
				...
				for(FlagPool flagPool : ormEntity.getPositiveFlagPoolList().values()){
					flagPool.query(ormEntity.getPrimaryKeyName(), ormEntity.getPrimaryKeyValue());
					html += "<tr><td colspan=\"2\">" + CheckListViewer.getHtmlCode(flagPool) + "</td></tr>";
				}
			}
			...
			if(promptData.getComplementaryInfoLineList().size() > 0){
				...
				for(String complementaryInfo : promptData.getComplementaryInfoLineList()){
			...
			html += getSingleAccessorLinkList((AbstractOrmEntity) data, promptData.isReadOnly());
			html += getMultipleAccessorLinkList((AbstractOrmEntity) data, promptData.isReadOnly());
		}
	...
	private static String getMultipleAccessorLinkList(AbstractOrmEntity entity, boolean isReadOnly) {
		String html = "";
		...
		for(String accessorName : entity.getPluralAccessorList().keySet()){
			...
			scrollList = new ScrollList(pluralAccessor.getInternalEntityDefinition().getVisibleName() + "(s)");

			ActionLink titleActionLink = new ActionLink("", new BaseAction("GetList",
			        pluralAccessor.getInternalEntityDefinition()));
			...
			for(AbstractOrmEntity currentForeignEntity : pluralAccessor){
				...
				scrollList.addLink(
				        ...
				        Servlet.makeLink(currentForeignEntity.getCurrentModule(), new BaseAction(actionName,
			...
			if(pluralAccessor.getInternalEntityDefinition().getFields().containsFieldName(entity.getForeignKeyName())
				...
				if(!isReadOnly){
					ActionLink newItemActionLink = new ActionLink(
					        ...
					        new BaseAction("New", pluralAccessor.getInternalEntityDefinition()));
					newItemActionLink.addParameters(entity.getForeignKeyName(), entity.getPrimaryKeyValue().toString());
					scrollList.addActionLink(newItemActionLink);
				}
			}

			html += "<tr><td style=\"column-span: all;\" colspan=\"100%\">"
			        + ScrollListViewer.getHtmlContent(scrollList) + "</td></tr>";
		}

		return html;
	}

	private static String getSingleAccessorLinkList(AbstractOrmEntity entity, boolean isReadOnly) {
		String html = "";
		...
		for(String accessorName : entity.getSingleAccessorList().keySet()){
			...
			String entityUrl = Servlet.makeLink(foreignEntity.getCurrentModule(), new BaseAction(actionName,
			        foreignEntity))
			        + "?" + foreignEntity.getPrimaryKeyName() + "=" + foreignEntity.getPrimaryKeyValue();

			String listUrl = Servlet.makeLink(foreignEntity.getCurrentModule(),
			        new BaseAction("GetList", foreignEntity));

			scrollList.addLinkGroup(listLinkName, listUrl, entityLinkName, entityUrl);
		}
